...
CREATE TABLE tbl_member(
   mno NUMBER(5),        -- 회원 번호 : 정수 최대 5자리
   name varchar2(50),    -- 이름 : 최대 50바이트, 한글 16문자
   email varchar2(100),  -- 이메일 : 최대 100바이트
   join_date DATE        -- 출력 형식 : yyyy-mm--dd HH24:MM:SS.ms
);						 -- 가입 날짜, 날짜 형식 : 날짜 패턴 문자열을 자동으로 날짜 형식 DATE 타입으로 변환

--예제 데이터
INSERT INTO 
	TBL_MEMBER -- 컬럼명 모두 생략
VALUES (1,'김모모','momo@naver.com','2022-11-24');
...
SELECT * FROM TBL_MEMBER tm WHERE NAME  = '박사나';

-- 4) 조건에 맞는 행의 지정된 컬럼 조회
SELECT email FROM TBL_MEMBER tm WHERE NAME = '박사나';

-- 5) order by로 정렬 컬럼 지정(가나다라 순서 또는 abcd 순서)
SELECT * FROM TBL_MEMBER tm ORDER BY name;   		-- 오름차 순
SELECT * FROM TBL_MEMBER tm ORDER BY name DESC;		-- 내림차 순
...
-- mno 값이 1,2,5,7 인 것 조회 : IN 연산(OR 연산)
