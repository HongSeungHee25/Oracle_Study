...
-- 외래키 옵션
-- 외래키 참조관계가 설정이되면 부모 테이블의 데이터가 삭제 시 오류
-- 옵션으로 다른 동작을 하도록 합니다. 

-- 외래키 옵션 테이스 테이블 생성(서브쿼리 이용에서 select 조회 결과를 복사해서 테이블 생성)
-- pk,fk 는 복사되지 않습니다.
...
-- 외래키 옵션 테이스를 위해서 기본키를 만듭니다.(기본키 여부 확인 필요) - alter 테이블 이용해서 추가하기
...
ADD CONSTRAINT score_pk 
PRIMARY KEY (stuno, subject);

-- 외래키 on delete 옵션 3가지 테스트 하기
-- 1. 아무것도 설정안하면 기본은 no action
ALTER TABLE TMP_SCORE 
ADD CONSTRAINT score_fk
...
-- 2. cascade(계단식의) 
-- 부모테이블을 참조하고 있는 자식테이블의 '같은 값을 참조하고 있는 행'을 삭제합니다.
ALTER TABLE TMP_SCORE 
ADD CONSTRAINT score_fk
FOREIGN KEY (stuno) REFERENCES tmp_student(stuno)
...
-- 3. set null
-- 외래키 컬럼이 기본키를 구성하는 컬럼 즉 식별관계일때는 동작 못합니다.
-- 비식별관계에서 외래키 컬럼이 NULL 허용이 된 경우에만 가능합니다.

-- 참조관계를 비식별관계를 하기 위해 기본키 컬럼 추가(기본 기본키 삭제하기)
ALTER TABLE TMP_SCORE 
