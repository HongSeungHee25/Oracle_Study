...
-- 회원가입 서비스를 예시로 하면, 회원 구별은 id(아이디) 또는 email 또는 회원번호
-- 회원을 구별(식별)하는 컬럼입니다. => not null 이면서 unique 해야합니다. => 기본키 PK(★★★★★완전 중요★★★★★)로 선정합니다.
...
-- 요구사항 예시 : 동일한 학생이 동일한 과목을 2번 수강/시험을 중복하지 않는다. 한 학생이 여러개의 과목을 수강한다.
...
-- FOREIGN KEY(성적테이블 컬럼) REFERENCES 참조테이블(참조테이블 컬럼)
-- 		* 학생테이블의 참조컬럼(stuno) 조건은 ? 기본키 또는 unique 컬럼이어야합니다. null 허용.
-- 		**참조관계를 만들 때 사용한 성적테이블 stuno는 값의 제약이 있습니다. tbl_student 테이블 stuno 컬럼에
-- 		 입력된 값으로 제한됩니다. '성적 테이블의 stuno 컬럼은 학생 테이블의 stuno 컬럼을 참조한다' 고 하며
-- 		 참조컬럼은 학생테이블의 stuno 이고, 외래키 컬럼은 성적테이블의 stuno 입니다.
);
-- 참조관계일때 자식 테이블 먼저 삭제 후 부모 테이블 삭제합니다.
...
-- 참고관계일때 자식 행 모두 제거 후 부모 테이블 제거 실행 오류입니다.
...
-- PK(행을 식별하는 역할) 컬럼을 조건으로 SELECT 조회하면 조회되는 행의 개수는 항상 0 또는 1입니다.
