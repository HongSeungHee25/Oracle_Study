-- // 1. 예시 : tbl_buy2 는 구매 목록 테이블 , tbl_money 는 결제 테이블 , custom_sale 는 고객매출 집계로 하여 
create table tbl_buy2           -- tbl_buy 구조만 복사. 시퀀스 생성은 안하고 임의로 값을 주고 테스트하겠습니다.
as
select * from tbl_buy where buy_seq=0;		-- 데이터 복사 안함

-- 트리거 활성화 ------------------------------------------------------------
ALTER TRIGGER set_money_trig enable;
-------------------------------------------------------------------------
...
VALUES (2000, 'mina012','CJ-BABQ1',3,sysdate,0);

-- //2. tbl_buy2 에 적용할 트리거
-- INSERT 는 :NEW, DELETE 는 :OLD, UPDATE 는 :NEW,:OLD 모두 있습니다.
CREATE  OR  REPLACE  TRIGGER  set_money_trig
AFTER  INSERT ON  tbl_buy2
--##1. AFTER  INSERT OR DELETE ON  tbl_buy2
FOR  EACH  ROW 
DECLARE 
    vprice number(7);
     vtotal number(7):=0;
    vcnt number(7):=0;
BEGIN 
--##2. DELETE FROM TBL_BUY2 tb WHERE buy_seq =; 했을때 :NEW.customid 출력되는지 확인해보기
   dbms_output.put_line('*' || :NEW.customid || ' ' || vtotal || ' ' || vcnt);
    ...
    INSERT  INTO  tbl_money VALUES (tblmoney_seq.nextval, :NEW.buy_seq,:NEW.customid ,:NEW.qty * vprice);
...
-- //5. 데이터 행 입력
INSERT INTO ICLASS.tbl_buy2 (BUY_SEQ,CUSTOMID,PCODE,QTY,BUY_DATE) 
