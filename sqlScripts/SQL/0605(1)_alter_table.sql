...
CREATE TABLE tbl_test2# (
	...
	pname varchar2(10) CONSTRAINT tname_notnull NOT NULL ,
	etc varchar2(10),
	tno number(3)
);
...
-- tid 의 길이를 20으로 변경하기
ALTER TABLE tbl_test# MODIFY (tid varchar2(20));		-- 컬럼레벨 제약조건 설정
-- 컬럼레벨 제약조건 설정. 'constraint 제약조건이름 not null'에서 생략된 형태입니다.

-- 없던 제약조건 추가는 실행.
ALTER TABLE tbl_test# MODIFY (tname varchar2(25) UNIQUE); 			-- UNIQUE -> 유일성(값 중복안됨)
...
-- add constraint 제약조건 이름 [제약조건 키워드]
...
INSERT INTO "TBL_TEST#" t (tid,tname)
VALUES ('tes','테스트2');			-- 중복된 tid 값 입력해보기 : 정상 실행

-- 위 PK 추가 하면 오류 : 중복된 값을 갖고 있으면 PK 추가 못합니다.

-- PK 없는 처음 상태에서 테스트하기
-- 이미 있는 not null 제약조건도 잘 실행됩니다.(제약조건 이름이 다르기때문 입니다.)
ALTER TABLE "TBL_TEST#" 
